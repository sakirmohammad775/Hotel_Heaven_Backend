...
class CartViewSet(
    ...
    def get_queryset(self):
        return Cart.objects.prefetch_related("items__hotel").filter(
...
def payment_cancel(request):
    return HttpResponseRedirect(f"{main_settings.FRONTEND_URL}/payment/cancelled")
...
class HasBookedHotel(APIView):
    ...
    def get(self, request, hotel_id):
        ...
        has_booked = Booking.objects.filter(
            user=user, items__hotel_id=hotel_id
        ).exists()
